...
; #LINKED ARRAYS - ARMOR ============================================================================
; Armor-typed counterparts of the LinkedArray*Form functions above. Papyrus arrays are not
; covariant - an Armor[] cannot be passed where a Form[] is declared - and Frostfall's
; _Frost_LegacyArmorDatastore keeps its Frostfall 3.0 data in quartets of Armor[128], so it needs
; these. They are a Campfire addition: the CheskoPapyrusShared repository carries only the Form
; versions, while the Frostfall 3.4 release bytecode calls
;     LinkedArrayHasArmor(armor, a1, a2, a3, a4)
;     LinkedArrayAddArmor(armor, a1, a2, a3, a4, true)
;     LinkedArrayRemoveArmor(armor, a1, a2, a3, a4, true)
; so these signatures keep both that bytecode and a fresh compile resolvable. The trailing bool on
; Add follows ArrayAddFormXT(..., bool bMustBeUnique = true) in Frostfall; on Remove it is
; LinkedArrayRemoveForm's abSort.

bool function LinkedArrayAddArmor(Armor akArmor, Armor[] akArray1, Armor[] akArray2, Armor[] akArray3, Armor[] akArray4, bool abMustBeUnique = true) global
	; Adds an armor to the first available element of the first array with room.

	; Return values
	; false 	=               Error (linked array full, or already present and abMustBeUnique)
	; true		=               Success

	if abMustBeUnique && LinkedArrayHasArmor(akArmor, akArray1, akArray2, akArray3, akArray4)
		return false
	endif

	int idx = akArray1.Find(none)
	if idx != -1
...
bool function LinkedArrayRemoveArmor(Armor akArmor, Armor[] akArray1, Armor[] akArray2, Armor[] akArray3, Armor[] akArray4, bool abSort = true) global
	; Removes the first occurrence of an armor from the arrays, if found.

	; Return values
	; false 	=		Error (Armor not found)
	; true		=		Success

	int idx = akArray1.Find(akArmor)
	if idx != -1
		akArray1[idx] = none
		if abSort
			LinkedArraySortArmor(akArray1, akArray2, akArray3, akArray4)
		endif
		return true
	endif

	idx = akArray2.Find(akArmor)
	if idx != -1
		akArray2[idx] = none
		if abSort
			LinkedArraySortArmor(akArray1, akArray2, akArray3, akArray4)
...
bool function LinkedArrayHasArmor(Armor akArmor, Armor[] akArray1, Armor[] akArray2, Armor[] akArray3, Armor[] akArray4) global
	; Attempts to find the given armor in the arrays, and returns true if found.

	; Return values
	; false		= 		Armor not found
	; true 		=		Armor found

	if akArray1.Find(akArmor) != -1
		return true
	elseif akArray2.Find(akArmor) != -1
		return true
	elseif akArray3.Find(akArmor) != -1
		return true
	elseif akArray4.Find(akArmor) != -1
		return true
	else
		return false
	endif
...
bool function LinkedArraySortArmor(Armor[] akArray1, Armor[] akArray2, Armor[] akArray3, Armor[] akArray4) global
	; Removes blank elements by shifting every armor down over the blanks before it, moving
	; elements to earlier arrays as needed, so the used elements form one contiguous run from
	; akArray1[0]. The same result as LinkedArraySortForms, done as a single pass over the
	; combined index space instead of one recursive call per blank.

	; Return values
	; false 	=           No sorting required
	; true 		=           Success

	int end1 = akArray1.Length
	int end2 = end1 + akArray2.Length
	int end3 = end2 + akArray3.Length
	int end4 = end3 + akArray4.Length
	...
	while read < end4
		Armor current
		if read < end1
			current = akArray1[read]
		elseif read < end2
			current = akArray2[read - end1]
		elseif read < end3
			current = akArray3[read - end2]
		else
			current = akArray4[read - end3]
		endif

		if current
			if write != read
				if write < end1
					akArray1[write] = current
				elseif write < end2
					akArray2[write - end1] = current
				elseif write < end3
					akArray3[write - end2] = current
				else
					akArray4[write - end3] = current
				endif

				if read < end1
					akArray1[read] = none
				elseif read < end2
					akArray2[read - end1] = none
				elseif read < end3
					akArray3[read - end2] = none
				else
					akArray4[read - end3] = none
				endif
				moved = true
			endif
			write += 1
