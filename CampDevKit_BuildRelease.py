...
dirname = os.path.join(BUILD_ROOT, "CampDevKit " + version + " Release")
...
for entry in read_manifest(project_path("CampDevKitArchiveManifest.txt")):
    if entry.startswith(EXTERNALS_TAG):
        relative = entry[len(EXTERNALS_TAG):]
        source = project_path("CampDevKit", "RequiredExternals", relative)
    else:
        relative = entry
...
# Create release zip
make_release_zip(dirname, "CampDevKit_" + version.replace(".", "_") + "_Release")
print("Done!")
